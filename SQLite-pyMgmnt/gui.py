...
class MmntInterface(object):
    ...
    def OnClick(self,event):
        ...
        if self.tree.parent(self.tree.selection()[0]) != '':
            ...
            self.NameChange.set(self.databases[parent].tables[ChildIndex])
    ...
    def ChangeName(self):                                                #Currently not working as intended, looking into it at the moment
        ...
        if parent != '' or ChildIndex != '':
            ...
            try:
                self.databases[parent].cur.execute(u"ALTER TABLE {t} RENAME TO {n}".format(t=table, n=NewName))
...
#            Canvas.config(scrollregion=self.canvasf2.bbox('all'))                    
    ...
    def ShowResult(self, parent, command, canvas):
        ...
        oldValue = len(self.databases[parent].tables)
        self.databases[parent].cur.execute(command)
        self.databases[parent].con.commit()
        rows = self.databases[parent].cur.fetchall()
        try:
            ColNames = [tuple[0] for tuple in self.databases[parent].cur.description]
        except TypeError:
            ColNames = ''
        self.databases[parent].cur.execute("SELECT * FROM sqlite_master WHERE type='table'")
        newValue = len(self.databases[parent].cur.fetchall())
...
#                time.sleep(0.002)    
    ...
    def TreeviewUpdate(self):
        if self.databases != []:
            id = str(len(self.databases))
            self.tree.insert('', 'end', id, text=self.databases[-1].dbName)
            for value in self.databases[-1].tables:
                self.tree.insert(id, 'end', text=value)

    def changeTables(self, parent):
        parentIndex = self.getParent()[1]
        self.databases[parent].tables = []
        
        self.databases[parent].cur.execute("SELECT * FROM sqlite_master WHERE type='table'")
        rows = self.databases[parent].cur.fetchall()
        if rows != []:
            for row in rows:
                self.databases[parent].tables.append(row[1])
        
        self.tree.delete(parentIndex)
        self.tree.insert('', 'end', parentIndex, text=self.databases[parent].dbName)
        
        for value in self.databases[parent].tables:
    ...
    def LoadDB(self, Type):
        ...
        try:
            ...
            try:
                ...
                self.error('error','not a database',1)
            except TypeError:
                pass
            except:
                self.error('error','could not display warning window',151)
        if connect == True:
            try:
                self.databases.append(dbCon.database(filename))
                index = len(self.databases) -1

                self.databases[index].cur.execute("SELECT * FROM sqlite_master WHERE type='table'")
                rows = self.databases[index].cur.fetchall()
                if rows != []:
                    for row in rows:
                        self.databases[index].tables.append(row[1])
            except:
                self.error('error','could not get tables from db',160)                
            self.TreeviewUpdate()
